...
# ── Import des templates pour charger le registry ─────────────────────────────
@st.cache_resource
def _load_all():
    import importlib
...
tests_map = all_tests()  # {test_id: class}
...
st.caption(f"{len(tests_map)} tests disponibles")
st.markdown("---")

# ── Recherche ─────────────────────────────────────────────────────────────────
s1, s2 = st.columns([3, 2])
search_q = s1.text_input("🔍 Rechercher", placeholder="ex: épaule, Berg, ODI…")
ai_q     = s2.text_input("🤖 Décrire le patient", placeholder="ex: patient âgé qui chute…")
...
with col_list:
    ...
    for tid, cls in filtered:
        ...
        if st.button(cls.tab_label(), key=f"bib_{tid}",
...
with col_detail:
    if not selected_tid or selected_tid not in tests_map:
        ...
        if _qk and _qk in QUESTIONNAIRES:
            ...
            if st.button("🖨️ Générer fiche PDF", key=f"gen_{selected_tid}"):
                with st.spinner("Génération…"):
                    try:
                        st.session_state[_pdf_key] = generate_questionnaires_pdf([_qk], {})
                    except Exception as e:
                        st.error(f"Erreur : {e}")
