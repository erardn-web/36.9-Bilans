...
class ASES(BaseTest):
    ...
    def render_evolution(cls, bilans_df, labels,
        ...
        if show_print_controls:
            _key = cls._print_chart_key('ases', cas_id)
            cls._render_print_checkbox(_key)
            cls._store_chart(_key, fig, cas_id)
        table_rows = [
            {"label": "ASES", "col_key": "ases_score",
             "values": [r.get("ases_score","—") for _,r in bilans_df.iterrows()]},
            {"label": "Résultat", "col_key": "ases_interpretation",
             "values": [r.get("ases_interpretation","—") for _,r in bilans_df.iterrows()]},
        ]
        if show_print_controls:
