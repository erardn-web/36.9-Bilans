...
@register_test
class VISAA(_VISABase):
    _PREFIX = "visa_a"
    @classmethod
    def meta(cls):
        return {"id":"visa_a","nom":"VISA-A — Tendinopathie Achille","tab_label":"🦶 VISA-A","categorie":"questionnaire",
                "tags":["achille","tendinopathie","cheville","sport"],
                "description":"VISA-A — Victorian Institute of Sport Assessment Achilles — /100"}
    @classmethod
    def fields(cls):
        return [f"visa_a_q{i}" for i in range(1,9)]+["visa_a_score"]

    def render(self, lv, key_prefix):
        st.markdown('<div class="section-title">🦶 VISA-A — Tendinopathie Achille</div>', unsafe_allow_html=True)
        st.markdown('<div class="info-box">Score 0 = incapacité totale · 100 = asymptomatique</div>', unsafe_allow_html=True)
        collected={}; total=0
        q_labels = [
            "Raideur au talon le matin (min avant diminution) — 0=+100min, 10=0min",
            "Douleur tendon Achille en étirement pendant 10s",
            "Descendre une marche (douleur /10)",
            "Sauter sur une jambe 10× sans douleur",
            "Exercice à plat (douleur)",
            "Exercice en côte (douleur)",
            "Activité sportive quotidienne",
            "Sport complet (semaines à niveau complet)",
        ]
        ...
        collected["visa_a_score"]=min(total,100)
        color="#388e3c" if total>=80 else "#f57c00" if total>=50 else "#d32f2f"
        st.markdown(f'<div class="score-box" style="background:{color}">VISA-A : {min(total,100)}/100</div>',unsafe_allow_html=True)
        return collected
...
@register_test
class VISAH(_VISABase):
    _PREFIX = "visa_h"
    @classmethod
    def meta(cls):
        return {"id":"visa_h","nom":"VISA-H — Tendinopathie ischio-jambiers","tab_label":"🦵 VISA-H","categorie":"questionnaire",
                "tags":["ischio-jambiers","hanche","tendinopathie","sport"],
                "description":"VISA-H — Victorian Institute Hamstring Tendinopathy — /100"}
    @classmethod
    def fields(cls):
    ...
    def render(self, lv, key_prefix):
        ...
        collected={}; total=0
        q_labels=["Douleur assise pendant 10 min","Douleur en marchant en terrain plat",
            "Douleur en courant","Douleur en sprint","Douleur lors d'entraînements spécifiques",
            "Douleur après effort","Activité complète","Pratique sportive pleine (semaines)"]
        for i,label in enumerate(q_labels,1):
            k=f"visa_h_q{i}"
            raw=lv(k,None)
            try: v=int(float(raw)) if raw not in (None,"","None") else 0
            ...
            val=st.slider(label,0,10,v,key=f"{key_prefix}_{k}")
            collected[k]=val; total+=val
        collected["visa_h_score"]=min(total,100)
        color="#388e3c" if total>=80 else "#f57c00" if total>=50 else "#d32f2f"
        st.markdown(f'<div class="score-box" style="background:{color}">VISA-H : {min(total,100)}/100</div>',unsafe_allow_html=True)
        return collected


@register_test
class VISAG(_VISABase):
    _PREFIX = "visa_g"
    @classmethod
    def meta(cls):
        return {"id":"visa_g","nom":"VISA-G — Tendinopathie fessière","tab_label":"🦴 VISA-G","categorie":"questionnaire",
                "tags":["grand trochanter","fessier","tendinopathie","hanche"],
                "description":"VISA-G — Victorian Institute Gluteal Tendinopathy — /100"}
    @classmethod
    def fields(cls):
    ...
    def render(self, lv, key_prefix):
        ...
        collected={}; total=0
        q_labels=["Douleur assise pendant 10 min","Douleur en marchant sur terrain plat",
            "Douleur en montant les escaliers","Douleur en position unipodale",
            "Douleur en courant","Douleur en sautant","Niveau d'activité complet",
            "Pratique sportive pleine (semaines)"]
        for i,label in enumerate(q_labels,1):
            k=f"visa_g_q{i}"
            raw=lv(k,None)
            try: v=int(float(raw)) if raw not in (None,"","None") else 0
            ...
            val=st.slider(label,0,10,v,key=f"{key_prefix}_{k}")
            collected[k]=val; total+=val
        collected["visa_g_score"]=min(total,100)
        color="#388e3c" if total>=80 else "#f57c00" if total>=50 else "#d32f2f"
        st.markdown(f'<div class="score-box" style="background:{color}">VISA-G : {min(total,100)}/100</div>',unsafe_allow_html=True)
        return collected
