...
def set_patient_archived(patient_id: str, archived: bool) -> bool:
    """Archive ou désarchive un patient (colonne 'archive' dans la feuille Patients)."""
    try:
        ss = ensure_sheets()
        ws = ss.worksheet("Patients")
        # S'assurer que la colonne 'archive' existe
        headers = ws.row_values(1)
        if "archive" not in headers:
            ws.update_cell(1, len(headers) + 1, "archive")
            headers.append("archive")
        archive_col = headers.index("archive") + 1  # 1-based

        all_values = ws.get_all_values()
        id_col = headers.index("patient_id")
        for i, row in enumerate(all_values[1:], start=2):
            if row and len(row) > id_col and row[id_col] == patient_id:
                ws.update_cell(i, archive_col, "Oui" if archived else "")
                return True
        return False
    except Exception:
        return False
...
def get_all_patients(include_archived: bool = False) -> "pd.DataFrame":
    """Retourne les patients. Par défaut exclut les archivés."""
    import pandas as pd
    ss = ensure_sheets()
    ws = ss.worksheet("Patients")
    headers = ws.row_values(1)
    # Ajouter 'archive' si absent
    if "archive" not in headers:
        all_headers = PATIENTS_HEADERS + ["archive"]
    else:
        all_headers = headers
    df = _ws_to_df(ws, all_headers)
    if df.empty:
        return df
    if not include_archived:
        if "archive" in df.columns:
            df = df[df["archive"].fillna("") != "Oui"]
